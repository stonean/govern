...
When the runtime is not on `PATH`, walk the eight family check scripts directly:

1. Run `scripts/audit/check-zero.sh` — generator/lint precondition pass. Halt with exit 1 if it reports findings; do not run the family checks against known-stale generator output.
...
Each family script prints findings to stdout and exits `0` (no findings) or `1` (findings present). Aggregate across all families; `/audit`'s exit code is the logical OR.
