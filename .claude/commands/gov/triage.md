...
Use the session target from `.claude/gov-session.json` if set, but triage operates across all specs so a target is not required.
...
- Reference: §bug-handling, §scenarios, §brownfield-triage (constitution loaded by `/gov:target` — do not re-read).
...
2. Walk the bug decision tree:
   ...
   **Step 1: Does a spec exist for this behavior?**
   ...
   - If no spec exists — recommend creating one via `/gov:specify`. Ask the user whether to create the spec now or skip this item.
