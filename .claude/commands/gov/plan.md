...
8. Invoke `lint-markdown` (MCP: `gov-rt:lint-markdown`) a second time as the readiness gate's final check. Any violations surface as advisory findings the user resolves before running `/gov:implement`. Otherwise, follow the markdown-only path.
