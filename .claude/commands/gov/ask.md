...
4. Update `.claude/gov-session.json` to set the new scenario as the session target (host responsibility — the runtime exposes no session-shaped primitive). Use tempfile + rename atomic-write semantics.
