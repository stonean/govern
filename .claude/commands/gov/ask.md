...
Read `.claude/gov-session.json`. If the session includes a `scenario` and `scenarioPath`, the target artifact is the scenario file. Otherwise, the target artifact is the feature's `spec.md`. If that file does not exist, stop and report: "Spec does not exist. Run `/gov:specify` first."
...
- Reference: §spec-requirements, §spec-lifecycle, §text-first-artifacts (constitution loaded by `/gov:target` — do not re-read).
...
> Spec is `done`. Run `/gov:elaborate` to capture this as a scenario instead.

A question on a `done` spec means either the behavior needs lower-level elaboration (a scenario, owned by `/gov:elaborate`) or the spec is wrong (manual revision). `/gov:ask`'s back-edge does not cover either. No question is recorded; no status mutation occurs.
...
- If a question was recorded (on a spec or a scenario), display: "Question recorded. Run `/gov:clarify` to resolve it." On a spec, the status is now `draft` regardless of where it started; on a scenario, `/gov:clarify` resolves the scenario's open questions per spec 009.
