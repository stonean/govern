...
8. Display:
   ...
   - Next pipeline step (based on status):
     ...
     - `done` → ask the user if they want to reopen this spec. If yes, update the spec's frontmatter `status` field to `in-progress` and suggest `/gov:elaborate` to capture the change. If no, confirm the spec is complete.
