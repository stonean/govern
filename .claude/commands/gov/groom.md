...
4. **Scenario route (scenario file)** — invoke `create-scenario` to write `specs/{feature}/scenarios/{slug}.md` from the scenario template (`specs/templates/scenario.md` framing: the `section:` frontmatter, the H1-from-slug, and the Open / Resolved Questions scaffolding). (`/gov:groom` keeps the inbox flow moving; for a deeper interactive walk through a single scenario, run `/gov:amend` against the parent spec — the classifier routes the input to the scenario branch.)
