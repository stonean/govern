...
4. **Scenario route** — invoke `create-scenario` to write `specs/{feature}/scenarios/{slug}.md` from the scenario template (`specs/templates/scenario.md` framing: the `section:` frontmatter, the H1-from-slug, and the Open / Resolved Questions scaffolding), then invoke `append-task` to append a task to the spec's `tasks.md` referencing the new scenario — the same two writes `/gov:amend`'s scenario route performs. (`/gov:groom` keeps the inbox flow moving; for a deeper interactive walk through a single scenario, run `/gov:amend` against the parent spec — the classifier routes the input to the scenario branch.)

5. **Reopen a `done` spec** — when the scenario route matched a spec whose status is `done`, invoke `set-status` with `from: done`, `to: in-progress` as part of the same routing action (the documented back-edge §spec-lifecycle "Backward via new scenario" defines, the same one `/gov:amend`'s scenario route performs). The `from` guard confirms the on-disk status is still `done` immediately before writing; if it has changed since the match (a concurrent edit), the write refuses — surface the discrepancy rather than blindly overwriting. Only from `done`: a matched spec in `draft`, `clarified`, `planned`, or `in-progress` is left unchanged — groom never advances or otherwise moves a non-`done` status. The spec-edit route is out of scope for this reopen (see **Reopening a `done` spec** below).

6. **Set the session target** — when the item routed to an existing spec, invoke `write-session`: for a confirmed spec edit, pass the matched feature and its repo-relative `specs/{feature}` directory; for a scenario route, additionally pass the new scenario slug and `specs/{feature}/scenarios/{slug}.md`. The primitive rewrites `.govern.session.toml` atomically, preserving any `cli-config-dir` already in the file (see **Setting the session target** below for the write shape). Rule items, missing-spec items (routed to `/gov:specify`), chores, and discards set no target. Across a multi-item run each spec-routed item performs this write, so the session target follows the current item and ends pointing at the most recently groomed spec.
