...
2. Invoke `read-spec` against the target feature (with `include-body`) and branch on the pair `(status, open-question count)` per the Gate table above — the result's frontmatter carries the status and its open-questions list carries the count (the Gate's entry-counting rule; placeholder lines are not entries):
   ...
   - `done` (any question count): stop with: "Spec is `done`. Run `/gov:amend` to capture this as a scenario instead." Exit without mutation.
...
9. Present a summary of changes and ask the user to approve the transition to `clarified`. Do not update the status until the user confirms. On confirmation, invoke `set-status` with `from: draft`, `to: clarified` against the feature, then display the next step: "Run `/gov:plan` to create the technical plan."
