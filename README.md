...
| [028-antigravity-agent](specs/028-antigravity-agent/spec.md) | planned | 012, 022 | Generalize the agent registry from "two agents that share one layout" to "N agents across differing per-agent layouts and host conventions," then use that generalization to add Google's **Antigravity CLI** (`agy`) as the third supported agent. |
