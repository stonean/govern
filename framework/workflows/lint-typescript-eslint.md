...
1. Detect the project's ESLint config — look for `eslint.config.{js,ts,mjs,cjs}` (flat config) or `.eslintrc*` (legacy). If none is found, warn the user but continue — ESLint will surface the missing-config error itself.
