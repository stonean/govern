...
- Do NOT create scenarios — the user runs `/{project}:elaborate` separately to decompose.
...
7. Fill in the spec from the user's description:
   ...
   - Leave frontmatter `tags` as `[]`; brownfield capture is intentionally sparse, and tags can be backfilled organically as the spec gains precision through `/{project}:clarify`.
...
11. Display the post-capture message:
    ...
    >
    > - `/{project}:elaborate` — capture a bug or edge case
