...
2. If a session target exists, read **only** the target spec's `spec.md` (or `spec-and-plan.md`) to get its status.
   - If the target spec's status is **not** `done`: display the target feature name and status. If a scenario is targeted, also read the scenario file and display scenario detail: scenario name, spec-ref, context summary, and open question count. Then prompt the next pipeline command based on status (`draft` → `/{project}:clarify`, `clarified` → `/{project}:plan`, `planned`/`in-progress` → `/{project}:implement`). **Stop here — do not build the full dashboard.**
