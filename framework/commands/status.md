...
<!-- audit:ignore-promotion -->
1. Render the **dashboard table** with one row per entry in the `specs` array. Columns: Feature, Status, Plan, Tasks, Data-model, Scenarios, Next Action. Mark the row matching the session target with a leading `>>`. The Scenarios column shows scenarios-count. The Next Action column comes from the Status → next action table below; when the status is in {clarified, planned, in-progress} and open-question-count is at least one, the Next Action is `clarify (recovery)` regardless of status — that recovery state usually arises from a manual frontmatter edit.
