...
- Reference: §spec-requirements, §text-first-artifacts (constitution loaded by `/{project}:target` — do not re-read).
...
If the spec status is `clarified` or later, warn the user: "This spec is already clarified. Adding an open question means it will need to go through `/{project}:clarify` again before advancing." Do not change the status — this is informational only.

### Prompt for another

Ask: "Do you have another question to add?" If yes, loop back to **Gather the question**. If no, display the next step:

- If spec status is `draft`: "Run `/{project}:clarify` when ready to resolve open questions."
- If spec status is `clarified` or later: "Run `/{project}:clarify` to re-resolve open questions before proceeding."
