...
These checks span the project's installed command set and constitution rather than the target feature. They catch drift in the framework files `govern` ships, surfaced per the Drift Prevention principles in `.govern/constitution.md` §drift-prevention. Run once per `/{project}:analyze` invocation regardless of which feature is targeted; with `--all`, run once before per-feature output.

Read inputs:

- `.govern/constitution.md` (already loaded by `/{project}:target`)
