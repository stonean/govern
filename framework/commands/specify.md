...
If the constitution has not been loaded in this session (e.g., `/{project}:target` has not been run), read `.govern/constitution.md` now to load `govern` rules. If the constitution was already loaded by `/{project}:target`, do not re-read it.
