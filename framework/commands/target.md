...
5. Invoke `run-generator` against the dependency generator as a safety net; when it reports drift, run the generator for real (host-mediated) to sync the frontmatter dependencies from body inline links. The pre-commit hook normally keeps this in sync; this step catches uncommitted body edits. On the markdown-only path, run `scripts/gen-spec-deps.sh --dry-run` by hand and apply the same rule on a diff.

6. Invoke `read-spec` against the resolved feature to load frontmatter, sections, and the open-question count from the body. The frontmatter status is one of draft, clarified, planned, in-progress, or done.
