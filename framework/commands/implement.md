...
- Reference: §implement-phase, §pipeline-boundaries, §text-first-artifacts, plus the rule-file directory's `configuration-cross.md` (`specs/rules/configuration-cross.md` in adopter projects; `framework/rules/configuration-cross.md` in govern's own repo) for constants and env-vars (constitution loaded by `/{project}:target` — do not re-read).
...
- Read `{cli-config-dir}/{project}-session.json` for the session target, including optional `scenario` and `scenarioPath` fields.
