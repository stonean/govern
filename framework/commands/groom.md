...
4. **Scenario route (scenario file)** — invoke `create-scenario` to write `specs/{feature}/scenarios/{slug}.md`. The primitive frames it with the `section:` frontmatter, the H1-from-slug, and the Open / Resolved Questions scaffolding (this framing is compiled into the primitive, mirroring `framework/templates/spec/scenario.md`; it does not read the template file — so an adopter who customizes `specs/templates/scenario.md` sees the compiled framing on the runtime path and the on-disk template only on the markdown-only path). (`/{project}:groom` keeps the inbox flow moving; for a deeper interactive walk through a single scenario, run `/{project}:amend` against the parent spec — the classifier routes the input to the scenario branch.)
