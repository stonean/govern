...
pub fn run(args: &MarkTaskArgs, repo: &Path) -> Result<CheckboxToggleResult> {
    ...
    let task_range = locate_task_range(&lines, &args.task_number).ok_or_else(|| {
...
/// Locate the `## <task_number>. ...` heading line and return the half-open
/// range of lines that constitute the task body (heading inclusive, next
/// `##` exclusive). Returns `None` when no matching heading is found.
fn locate_task_range(lines: &[&str], task_number: &str) -> Option<std::ops::Range<usize>> {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        } else if level == 2 && heading_task_number(&heading).as_deref() == Some(task_number) {
            start = Some(idx);
        }
    }
