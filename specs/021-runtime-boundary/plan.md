...
- **Implementing the fallback lint as a structured-marker check (e.g., `<!-- runtime-tool: name --> ... <!-- /runtime-tool -->`)** — rejected. Structured markers depend on author discipline, exactly the anti-pattern §design-principles in `AGENTS.md` forbids. The proximity scan is derived from existing command prose and requires no author markup.
