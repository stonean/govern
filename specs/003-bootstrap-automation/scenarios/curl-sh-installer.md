...
- It fetches `framework/bootstrap/govern.md` from the ref named by `GOVERN_REF` (default `main`), so an adopter can pin a tagged release with `… | GOVERN_REF=v0.1.0 sh` — consistent with the live-on-main model.
