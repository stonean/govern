...
**Verdict: adapt.** Opt-in via `/gov:implement --auto`, default off. Per-invocation flag (not session state, not spec frontmatter) — decision happens in execution context, not selection context, and matches CLI convention (flags modify the verb). `gov-session.json` does not gain an `autoAdvance` field.
...
`gov-session.json` keeps holding one target. Documentation gains a one-paragraph note (in the constitution or `AGENTS.md` template) directing users to `git worktree` and platform isolation for concurrent feature work.
