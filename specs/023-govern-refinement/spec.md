...
### 3. `/elaborate` consolidated into `/ask`

`/ask` becomes the single entry point for adding to a targeted spec. During the existing refinement loop in `ask.md`, `/ask` classifies the input as a question or a scenario:
...
The classification surfaces during the user-approves-the-refined-form gate that already exists in `/ask` — the user can override the framework's choice with one click.

The current `/ask` refusal on `done` specs goes away: scenario routing handles `done` specs natively.
