...
- [x] Tag-push `gvrn-v0.13.0` — the `runtime-release.yml` workflow auto-builds the matrix and publishes to crates.io on the tag; requires user authorization (externally visible); shared with §43
...
- [x] Tag-push `gvrn-v0.13.0` — the `runtime-release.yml` workflow auto-builds the matrix and publishes to crates.io on the tag; requires user authorization (externally visible); shared with §42, §44
...
- [x] Tag-push `gvrn-v0.13.0` — the `runtime-release.yml` workflow auto-builds the matrix and publishes to crates.io on the tag; requires user authorization (externally visible); shared with §42, §43
...
## 46. Implement scenario: [review-exec-wiring](scenarios/review-exec-wiring.md)

Follow-on to task 45: `/gov:review` works on the MCP/host path but not yet under `runtime exec`. The interpreter walker discards primitive results (`handle_primitive` → `Ok(_) => Ok(None)`), so on the exec path the review procedure's later steps do not receive `compute-review-scope`'s scope or `discover-rule-files`'s rule set. Surfaced during 45i; deferred here so the cross-command golden re-bless is deliberate.

- [x] **46a — thread primitive results into the walker context.** Merge each dispatched primitive's structured result's top-level keys into the walker context (`runtime/src/interpreter/mod.rs`), with an explicit merge policy that preserves session-seeded keys (e.g. `write-boundary`) or namespaces results. Add walker tests asserting `compute-review-scope`'s `scope`/`diff-base` and `discover-rule-files`'s `selected`/`rules-dir` reach `build_perform_review_request` and `write-review`. Done when the walker ABI tests are green.
- [x] **46b — `/gov:review` exec golden / parity fixture.** Add a git-backed fixture (spec + rule files + plan + `stdin.jsonl` of `performReview` responses) and a parity golden asserting the full stream (`compute-review-scope` → `discover-rule-files` → `process-waivers` → 5× `performReview` with populated scope/rules → `write-review` → `complete`), mirroring the `implement-basic` fixture shape and the `{{runtime-version}}` placeholder. Done when the review parity test is green.
