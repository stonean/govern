...
- **`process-waivers`** — apply; expire (file gone); expire (rule no longer fires); retain-not-expire on a dimension-restricted run (`skipped-passes` non-empty); do-not-extend to another file; malformed (missing field → skip-and-warn, not pruned); duplicate (first applies, dup warns); code-moved-within-file (anchor is the `(rule, file)` pair, not the line → still applies).
...
- **`write-review`** — empty-scope 0-findings / `blocking: false` report; cross-pass dedup (highest-severity-wins on rule-id + file + overlapping range); `blocking` true when `must-violations` exceeds zero; `skipped-passes` recorded under the dimension flags; the frontmatter `review:` block fields (`last-run`, `reviewed-against`, `must-violations`, `should-violations`, `low-confidence`, `blocking`); adopter-authored waiver fields preserved verbatim on surviving waivers (open-schema round-trip); single-`findings`-array ingestion.
