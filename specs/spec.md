...
- **Auggie permissions setup** — the `/gov:setup` command configures per-project permissions for Claude Code via `.claude/settings.local.json`. Auggie's permissions model is global (`~/.augment/settings.json`), so modifying it during project adoption is risky. For now, the Auggie govern variant skips permissions setup. Revisit when Auggie supports per-project permission overrides or when a safe pattern for global config modification emerges.
