...
| --- | --- | --- |
| 1 | `title:` PKM frontmatter | `framework/templates/spec/{spec,plan,tasks,data-model,research,scenario,spec-and-plan}.md`; written by `/specify`, `/plan`, `/ask`; checked + auto-fixed by `/analyze` |
| 2 | `track: lightweight` field | `framework/templates/spec/spec-and-plan.md` only |
| 3 | `tags` frontmatter | Constitution §text-first-artifacts schema; prompted by `/specify`; checked advisory by `/clarify` and `/analyze`; "Starter Tag Vocabulary" table in constitution |
...
| --- | --- | --- |
| 9 | `spec-ref` parent half in scenario frontmatter | Parent feature is the directory the scenario lives in; scenario frontmatter shrinks to the section name only (`section:` field), and `/ask` (scenario branch) writes it. |
...
| # | Violation | Design |
| --- | --- | --- |
| 18 | §constants + §env-vars rules | Move both constitution sections into a new rule file at `framework/rules/configuration-cross.md` with rules carrying RFC 2119 statements + Verification steps that `/analyze` runs against feature artifacts. The constitution sections shrink to a one-line pointer ("See `framework/rules/configuration-cross.md`"). Adoption: shipped to adopters via `/govern` like other rule files. |
...
- [x] AC9: `/specify`, `/plan`, and `/ask` each write the canonical filename-derived metadata (formerly `title:`) at scaffold time without prompting the author
...
- [x] AC15: `/analyze` no longer offers `--fix` for checkbox state; the Fix Mode section is removed and the checkbox-related entries in the Fixable list are gone
