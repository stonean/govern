...
#
# Allowlist: a line preceded by `<!-- audit:ignore-placeholders -->` on the
# previous (non-blank) line is skipped. Primary case is
# framework/commands/audit.md itself — `/audit` is maintainer-only and not
# scaffolded into adopters, so its self-references stay literal.
#
